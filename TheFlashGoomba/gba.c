...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  lcolor = color; //for later use for assigning a block of color to one row
  for (int r = 0; r < height; r++) { //generate in DMA by row using height calls
    DMA[3].src = &lcolor; //source to the color we want
    DMA[3].dst = &videoBuffer[OFFSET(row+r, col, WIDTH)]; //set the destination for the color to be applied
    //how much to copy -> width and then toggle off based on DMA settings
...
void drawFullScreenImageDMA(const u16 *image) {
  DMA[3].src = image; //source to the color we want
  DMA[3].dst = videoBuffer; //set the destination for the color to be applied, which is the start of the screen
  //how much to copy -> full screen size and then toggle off based on DMA settings
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int r = 0; r < height; r++) { //generate in DMA by row using height calls
    ...
    DMA[3].src = image + (r * width); //source to the color we want
    DMA[3].dst = &videoBuffer[OFFSET(row+r, col, WIDTH)]; //set the destination for the color to be applied
    //how much to copy -> width and then toggle off based on DMA settings
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT; 
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int r = 0; r < height; r++) { //generate in DMA by row using height calls
    ...
    DMA[3].src = &image[OFFSET(row+r, col, WIDTH)]; //source to the color we want
    DMA[3].dst = &videoBuffer[OFFSET(row+r, col, WIDTH)]; //set the destination for the color to be applied
    //how much to copy -> width and then toggle off based on DMA settings
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT; 
...
void fillScreenDMA(volatile u16 color) {
  ...
  lcolor = color;
  DMA[3].src = &lcolor; //source to the color we want
  DMA[3].dst = videoBuffer; //set the destination for the color to be applied
  //how much to copy -> width and then toggle off based on DMA settings
  DMA[3].cnt = WIDTH * HEIGHT | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT; 
}
